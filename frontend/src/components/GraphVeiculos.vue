...
const drawChart = () => {
    ...
    const margin = 5;
    // tamanho do gráfico
    const radius = (Math.min(width, height) / 2 - margin) * 0.7;
    ...
    g.selectAll('path')
        ...
        .join('path')
        .attr('d', arcGenerator)
        .attr('fill', d => colorScale(translateVehicleKey(d.data.key)))
        .attr('stroke', '#fff')
        ...
        .on('click', (event, d) => {
            ...
            const translatedKey = translateVehicleKey(d.data.key);
            const infoText = `${translatedKey}: ${d.data.value} registro(s) (${percentage}%)`;
